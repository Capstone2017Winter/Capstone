...
class RemoteLDAPDatabase(AbstractRemoteDatabase):
    ...
    def searchForAllSections(self, term, subject, catalog):
        path = 'term={},{}'.format(term, self._basedn)
        scope = ldap.SCOPE_ONELEVEL
        request_filter = '(&(subject={})(catalog={}))'.format(subject, catalog)
        attrs = self._saved_searches.get('catalog').get('attrs')
        course_number = ""


        ldap_result_id = self._client.search(path, scope, request_filter, attrs)
        result_set = []
        _, result_data, msgid, serverctrls = self._client.result3(ldap_result_id)
        course_number = extract_results_from_ldap_data(result_data)[0]['course']
        #except ldap.LDAPError, e:
         #   print e

        courses = self.search('sections', None, term, course_number)
        results = []

        for hit in courses:
            class_time = self.search('classtimes', None, term, course_number, hit['class'])[0]
            results.append(dict(hit, **class_time))
    ...
    def searchForInfo(self, term, subject, catalog):
        path = 'term={},{}'.format(term, self._basedn)
        scope = ldap.SCOPE_ONELEVEL
        request_filter = '(&(subject={})(catalog={}))'.format(subject, catalog)
        attrs = self._saved_searches.get('catalog').get('attrs')
        ...
        try:
            ...
            print e
        courses = self.search('courses', None, term, course_number)
        return courses
